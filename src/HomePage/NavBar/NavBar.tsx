...
import {updatesearchMovie,updategoogleUserName} from "../../Store/Slice.js"
...
export default function PrimarySearchAppBar(props) {
  ...
  let gstateUserName=stateSelect.googleUserName;
  ...
  //------------------ google-sigin -------------
 const [googleUserName, setgoogleUserName] = useState("Sign in");
    ...
    signInWithPopup(auth, provider)
      .then((result) => {
       ...
       // const token = credential.accessToken;
        ...
        let gusername=user.displayName;
       // setgoogleUserName(gusername);
        dispatch(updategoogleUserName(gusername));
  ...
  const googleSignOutFn=()=>{
const auth = getAuth();
  signOut(auth).then(() => {
    console.log("loggedout");
    let logoutVal="Sign in";
    dispatch(updategoogleUserName(logoutVal));
    // Sign-out successful.
  }).catch((error) => {
    // An error happened.
  ...
  const searchName=(e)=>{
    ...
    console.log(val);
    props.parentFn(val)
    //dispatch(updatesearchMovie(val));
  }
  return (
    <Box sx={{ flexGrow: 1 }}>
